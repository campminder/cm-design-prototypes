...
export const CampWebsite = ({ onPortalClick }) => {
  return (
    <div className="min-h-dvh flex flex-col bg-[#fafaf8] font-sans">
      ...
      <header className="flex items-center justify-between px-8 py-4 bg-white border-b border-gray-200">
        ...
        <div className="flex items-center gap-3">
          <button
            className="hidden sm:inline-flex bg-[#2d6a4f] hover:bg-[#245c44] text-white text-sm font-medium px-4 py-2 rounded-md transition-colors"
            onClick={onPortalClick}
          >
            myTallPines
          </button>
      ...
      >
        ...
        <div className="relative z-10 p-8 md:p-12 w-full max-w-3xl">
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <button className="bg-[#2d6a4f] hover:bg-[#245c44] text-white text-base font-semibold px-7 py-3 rounded-md transition-colors">
              Explore Programs
            </button>
            <button
              className="border border-white/60 text-white bg-white/10 hover:bg-white/20 backdrop-blur-sm text-base font-semibold px-7 py-3 rounded-md transition-colors"
              onClick={onPortalClick}
            >
              Parent Portal
            </button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 px-8 py-12 max-w-5xl mx-auto w-full">
        {[
          { emoji: '🏕️', title: 'Summer 2026', body: 'Registration is now open for new and returning families. Secure your spot today.', cta: 'Learn more →', onClick: null },
          { emoji: '👨‍👩‍👧', title: 'Returning Families', body: 'Log in to your parent portal to manage enrollment, view forms, and stay connected.', cta: 'myTallPines →', onClick: onPortalClick },
          { emoji: '✨', title: 'New Families', body: 'Start the enrollment process and learn what makes Tall Pines a home away from home.', cta: 'Get started →', onClick: null },
        ].map((card) => (
          <div key={card.title} className="rounded-xl border border-gray-100 shadow-sm hover:shadow-md transition-shadow bg-white p-6">
            <div className="text-3xl mb-3">{card.emoji}</div>
            <h3 className="text-lg font-semibold text-gray-900 mb-2">{card.title}</h3>
            <p className="text-sm text-gray-600 leading-relaxed mb-3">{card.body}</p>
            <button className="text-sm font-medium text-[#2d6a4f] hover:underline" onClick={card.onClick}>
              {card.cta}
            </button>
          </div>
        ))}
      ...
      <footer className="bg-gray-900 text-gray-400">
        <div className="max-w-5xl mx-auto px-8 py-12 flex flex-col md:flex-row justify-between gap-10">
          <div className="space-y-3">
            ...
            <p className="text-sm leading-relaxed">1247 Pine Ridge Road<br />Lake Placid, NY 12946</p>
            <p className="text-sm">(518) 555-0142 &bull; info@camptallpines.com</p>
          </div>
          <div className="grid grid-cols-3 gap-8 text-sm">
            {[
              { heading: 'Camp', links: ['About Us', 'Our Team', 'Facilities', 'Employment'] },
              { heading: 'Programs', links: ['Day Camp', 'Overnight', 'Teen Leadership', 'Specialty Camps'] },
              { heading: 'Families', links: ['Parent Portal', 'What to Pack', 'FAQs', 'Health & Safety'] },
            ].map((col) => (
              <div key={col.heading} className="flex flex-col gap-2">
                <h4 className="text-white font-semibold mb-1">{col.heading}</h4>
                {col.links.map((link) => (
